...
def test_concepts():
    ...
    dimensions = ["hue", "round", "sweet"]
    current_dims = list(dimensions)
    plot_dim_indices = [(0,1), (0,2), (1,2)]
    current_plot_dim_indices = list(plot_dim_indices)
    current_3d_indices = [0,1,2]
    ...
    concepts = {"apple":(apple, 'r', (0.05, 0.10, 0.10)), "banana":(banana, 'y', (0.05, 0.02, 0.15)), 
    ...
    fig.canvas.set_window_title("ConceptInspector")

    # set up 3d plot    
    ax_3d = fig.add_subplot(221, projection='3d')
    def init_ax_3d():
        first_dim = current_dims[0]
        second_dim = current_dims[1]
        third_dim = current_dims[2]
        ax_3d.clear()
        ax_3d.set_title("3D visualization - {0}, {1}, {2}".format(first_dim, second_dim, third_dim))
        ax_3d.set_xlabel(first_dim)
        ax_3d.set_xlim(-0.1, 1.1)
        ax_3d.set_ylabel(second_dim)
        ax_3d.set_ylim(-0.1, 1.1)
        ax_3d.set_zlabel(third_dim)
    ...
    ax_2d.append(fig.add_subplot(222))
    def init_ax_2d(idx):
        first_dim = current_dims[plot_dim_indices[idx][0]]
        second_dim = current_dims[plot_dim_indices[idx][1]]
        ax_2d[idx].clear()
        ax_2d[idx].set_title("2D visualization - {0}, {1}".format(first_dim, second_dim))
        ax_2d[idx].set_xlabel(first_dim)
        ax_2d[idx].set_xlim(-0.2,1.2)
        ax_2d[idx].set_ylabel(second_dim)
        ax_2d[idx].set_ylim(-0.2,1.2)
    init_ax_2d(0)

    # set up second 2d plot
    ax_2d.append(fig.add_subplot(223))
    init_ax_2d(1)

    # set up third 2d plot
    ax_2d.append(fig.add_subplot(224))
    init_ax_2d(2)
    ...
    def draw_concept(concept, color, epsilons):
        for cuboid in concept:
            # plot cuboid in 3d
            x, y, z = _box_data_3d(cuboid[0], cuboid[1], current_3d_indices)
            ax_3d.plot_surface(x, y, z, color=color, rstride=1, cstride=1, alpha=alpha_3d)
            
            # plot cuboid in 2d projections
            for i in range(3):
                idx = current_plot_dim_indices[i]
                ...
                ax_2d[i].add_patch(alpha_patch)
 
    
    active_concepts = list(concepts.keys())
    
    for concept_name in active_concepts:
        concept, color, epsilons = concepts[concept_name]
        draw_concept(concept, color, epsilons)
    
    # now add radio buttons for selecting dimensions
    fig.subplots_adjust(left=0.2, right=0.95)
    
    first_dim_radios_ax = fig.add_axes([0.025, 0.75, 0.1, 0.15], axisbg='w')
    first_dim_radios_ax.set_title("First dimension")
    first_dim_radios = RadioButtons(first_dim_radios_ax, dimensions, active=0)
    def first_dim_click_handler(label):
        current_dims[0] = label
        idx = dimensions.index(label)
        current_plot_dim_indices[0] = (idx, current_plot_dim_indices[0][1])
        current_plot_dim_indices[1] = (idx, current_plot_dim_indices[1][1])
        current_3d_indices[0] = idx
        init_ax_3d()
        init_ax_2d(0)
        init_ax_2d(1)
        init_ax_2d(2)
    ...
    second_dim_radios_ax = fig.add_axes([0.025, 0.55, 0.1, 0.15], axisbg='w')
    second_dim_radios_ax.set_title("Second dimension")
    second_dim_radios = RadioButtons(second_dim_radios_ax, dimensions, active=1)
    def second_dim_click_handler(label):
        current_dims[1] = label
        idx = dimensions.index(label)
        current_plot_dim_indices[0] = (current_plot_dim_indices[0][0], idx)
        current_plot_dim_indices[2] = (idx, current_plot_dim_indices[2][1])
        current_3d_indices[1] = idx
        init_ax_3d()
        init_ax_2d(0)
        init_ax_2d(1)
        init_ax_2d(2)
    ...
    third_dim_radios_ax = fig.add_axes([0.025, 0.35, 0.1, 0.15], axisbg='w')
    third_dim_radios_ax.set_title("Third dimension")
    third_dim_radios = RadioButtons(third_dim_radios_ax, dimensions, active=2)
    def third_dim_click_handler(label):
        current_dims[2] = label
        idx = dimensions.index(label)
        current_plot_dim_indices[1] = (current_plot_dim_indices[1][0], idx)
        current_plot_dim_indices[2] = (current_plot_dim_indices[2][0], idx)
        current_3d_indices[2] = idx
        init_ax_3d()
        init_ax_2d(0)
        init_ax_2d(1)
        init_ax_2d(2)
    ...
    concept_checks_ax = fig.add_axes([0.025, 0.1, 0.1, 0.15], axisbg='w')
    concept_checks_ax.set_title("Concepts")
    concept_checks = CheckButtons(concept_checks_ax, list(active_concepts), [True]*6)
    c = map(lambda x: x[1], list(concepts.values()))    # color them nicely
    [rec.set_facecolor(c[i]) for i, rec in enumerate(concept_checks.rectangles)]
    def concept_checks_click_handler(label):
        if label in active_concepts:
            active_concepts.remove(label)
        else:
            active_concepts.append(label)
        init_ax_3d()
        init_ax_2d(0)
        init_ax_2d(1)
        init_ax_2d(2)
